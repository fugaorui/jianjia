...
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations{
    ...
    if ([locationStr0 length]>0) {
        ...
        if (self.isGetLocation){
            ...
            NSDictionary *dataDic = @{@"latitude":@(newCoordinate2D.latitude),@"longitude":@(newCoordinate2D.longitude)};
            [self.euexObj.webViewEngine callbackWithFunctionKeyPath:@"uexLocation.cbGetLocation" arguments:ACArgsPack(@0,@2,dataDic.ac_JSONFragment)];
        }
        //世界标准地理坐标转化为百度坐标系
